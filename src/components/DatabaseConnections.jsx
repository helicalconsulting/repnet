...
function ConnectionCard({ connection, onSync, onSyncSchema, onGenerateAdapters, onDelete, isAdmin, isViewer, isActive, onActivate, isCollapsed, onToggleCollapse }) {
  ...
  const [tableHints, setTableHints] = useState({}); // { [tableName]: { hint: '', column_hints: { [colName]: '' } } }
  const [expandedHintTable, setExpandedHintTable] = useState(null);
  const [loadingModalColumns, setLoadingModalColumns] = useState(null);
  ...
  const toggleExpandHintTable = async (tableName) => {
    if (expandedHintTable === tableName) {
      setExpandedHintTable(null);
      return;
    }
    setExpandedHintTable(tableName);
    if (!columnsMap[tableName]) {
      setLoadingModalColumns(tableName);
      try {
        const cols = await getTableColumns(connection.id, tableName);
        setColumnsMap(prev => ({ ...prev, [tableName]: cols || [] }));
      } catch (err) {
        console.error("Failed to load columns for hints:", err);
      } finally {
        setLoadingModalColumns(null);
      }
    }
  };
  ...
  const handleGenerateAdapters = async (selectedTablesList = null, customHintText = "", tableHintsMap = null) => {
    setIsGeneratingAdapters(true);
    ...
    >
      ...
      {showAnalyzeConfig && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div 
            className={`bg-background border border-border/80 dark:border-white/10 rounded-2xl flex flex-col justify-between shadow-2xl transition-all duration-300 ${
              isMaximized ? 'w-[92vw] h-[85vh]' : 'w-full max-w-2xl h-[75vh]'
            }`}
          >
            {/* Header */}
            <div className="flex items-center justify-between border-b border-border/50 px-5 py-4 shrink-0">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-muted-foreground">Select tables and provide optional metadata hints to guide AI indexing</p>
                </div>
              </div>
              <div className="flex items-center gap-1.5">
                <button
                  onClick={() => setIsMaximized(!isMaximized)}
                  className="p-1.5 hover:bg-black/5 dark:hover:bg-white/10 rounded text-muted-foreground hover:text-foreground transition-colors"
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4 custom-scrollbar text-left">
              {/* Instructions note */}
              <div className="bg-blue-500/5 border border-blue-500/10 rounded-xl p-3 text-xs text-blue-400">
                <p className="font-semibold mb-1">💡 Professional Tips for Custom Hints:</p>
                <ul className="list-disc pl-4 space-y-1 text-[11px] text-muted-foreground/80">
                  <li>General Instructions apply globally to all mapping (e.g. "Focus on inventory and finance").</li>
                  <li>Table & Field hints target specific structures (e.g. explain that <code className="font-mono text-foreground px-0.5 bg-white/5 rounded">CustBal</code> tracks balances, or that <code className="font-mono text-foreground px-0.5 bg-white/5 rounded">Zip</code> is optional).</li>
                  <li>These hints are completely optional. Left empty, the AI will auto-detect context from names.</li>
                </ul>
              </div>

              {/* Custom guidance hints */}
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-foreground">General AI Instructions / Hints (Optional)</label>
                <textarea
                  value={customHint}
                  onChange={(e) => setCustomHint(e.target.value)}
                  placeholder="e.g. 'Ignore temporary/log tables', 'Company departments are in tbl_dept', 'Only focus on finance concepts'"
                  className="w-full text-xs p-3 rounded-xl border border-border/50 dark:border-white/5 bg-black/5 dark:bg-white/[0.03] focus:outline-none focus:ring-1 focus:ring-primary h-20 resize-none placeholder:text-muted-foreground/60 transition-all text-foreground"
                />
              </div>

              {/* Table checklist header with Search */}
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <label className="text-xs font-semibold text-foreground">
                    Select Tables & Provide Hints ({selectedTables.length} / {tables.length})
                  </label>
                ...
                <input
                  type="text"
                  placeholder="Search tables within checklist..."
                  value={modalSearchQuery}
                  onChange={(e) => setModalSearchQuery(e.target.value)}
                  className="w-full bg-black/5 dark:bg-white/[0.03] border border-border/40 dark:border-white/5 rounded-xl px-3 py-2 text-xs text-foreground focus:outline-none focus:border-primary/50 transition-colors"
                />

                {loadingTables ? (
                  <div className="flex items-center justify-center py-8 gap-2 text-xs text-muted-foreground">
                    <Loader2 className="w-4 h-4 animate-spin text-primary" /> Loading database tables...
                  </div>
                ) : (
                  <div 
                    className={`border border-border/50 dark:border-white/5 rounded-xl overflow-y-auto divide-y divide-border/30 dark:divide-white/5 custom-scrollbar transition-all ${
                      isMaximized ? 'h-[40vh]' : 'h-[25vh]'
                    }`}
                  >
                    {tables
                      ...
                      .map(table => {
                        ...
                        const isExpanded = expandedHintTable === table.name;
                        return (
                          <div key={table.name} className="flex flex-col bg-background">
                            <div className="flex items-center justify-between px-3 py-2 hover:bg-black/5 dark:hover:bg-white/[0.02]">
                                ...
                                <input
                                  ...
                                  onChange={() => {
                                    if (isChecked) {
                                      ...
                                      setTableHints(prev => {
                                        const copy = { ...prev };
                                        delete copy[table.name];
                                        return copy;
                                      });
                                      if (expandedHintTable === table.name) setExpandedHintTable(null);
                                    } else {
                              ...
                              {isChecked && (
                                <button
                                  onClick={() => toggleExpandHintTable(table.name)}
                                  className={`px-2 py-0.5 text-[10px] font-semibold border rounded-lg transition-all flex items-center gap-1.5 ${
                                    isExpanded 
                                      ? 'bg-primary/10 border-primary text-primary' 
                                      : 'bg-transparent border-border/50 text-muted-foreground hover:text-foreground hover:border-foreground/40'
                                  }`}
                                >
                                  <SlidersHorizontal className="w-3 h-3" />
                                  {isExpanded ? 'Hide Hints' : 'Configure Hints'}
                                </button>
                              )}
                            </div>

                            {isChecked && isExpanded && (
                              <div className="px-3 pb-3 pt-2 bg-black/5 dark:bg-white/[0.02] border-t border-border/30 dark:border-white/5 space-y-3">
                                <div className="space-y-1">
                                  <label className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider block">
                                    Table Hint / Context
                                  </label>
                                  <textarea
                                    value={tableHints[table.name]?.hint || ''}
                                    onChange={(e) => {
                                    ...
                                    placeholder="e.g. stores order history details, references Customer ID"
                                    className="w-full text-xs p-2 rounded-lg bg-background border border-border/50 dark:border-white/5 focus:outline-none focus:border-primary placeholder:text-muted-foreground/40 h-14 resize-none text-foreground"
                                  />
                                </div>
                                ...
                                <div className="space-y-1">
                                  ...
                                  {loadingModalColumns === table.name ? (
                                    <div className="flex items-center gap-1.5 text-[10px] text-muted-foreground py-1">
                                      <Loader2 className="w-3 h-3 animate-spin text-primary" /> Loading field list...
                                    </div>
                                  ) : (
                                    <div className="space-y-2 max-h-48 overflow-y-auto pr-1 custom-scrollbar">
                                      {(columnsMap[table.name] || []).map(col => (
                                        <div key={col.name} className="flex items-center gap-2 justify-between">
                                          <span className="font-mono text-[10px] text-foreground/80 truncate w-1/3 text-left" title={`${col.name} (${col.type})`}>
                                            {col.name}
                                          </span>
                                          <input
                                            type="text"
                                            placeholder={`Enter hint for ${col.name}...`}
                                            value={tableHints[table.name]?.column_hints?.[col.name] || ''}
                                            onChange={(e) => {
                                              const val = e.target.value;
                                              setTableHints(prev => {
                                                const tbl = prev[table.name] || {};
                                                const colHints = tbl.column_hints || {};
                                                return {
                                                  ...prev,
                                                  [table.name]: {
                                                    ...tbl,
                                            ...
                                            className="flex-1 text-[10px] px-2 py-1 rounded bg-background border border-border/50 dark:border-white/5 focus:outline-none focus:border-primary placeholder:text-muted-foreground/30 font-normal text-foreground"
                                          />
                                        </div>
                                      ))}
                                      {(columnsMap[table.name] || []).length === 0 && (
                                        <span className="text-[10px] text-muted-foreground italic block">No columns found. Try refreshing database schema.</span>
                                      )}
                                    </div>
                                  )}
                                </div>
                    ...
                    {tables.filter(t => t.name.toLowerCase().includes(modalSearchQuery.toLowerCase())).length === 0 && (
                      <div className="text-center py-6 text-xs text-muted-foreground">No matching tables found.</div>
                    )}
                  </div>
                )}
              </div>
            ...
            {/* Footer */}
            <div className="mt-auto border-t border-border/50 px-5 py-4 flex items-center gap-2 shrink-0">
              <button
                onClick={() => setShowAnalyzeConfig(false)}
              ...
              <button
                onClick={() => {
                  ...
                  handleGenerateAdapters(selectedTables, customHint, tableHints);
                }}
